...
class PortfolioManagerApplicationTest {
  ...
  void mainReadQuotes() throws Exception {
    ...
    // when
    List<String> actual =
        PortfolioManagerApplication.mainReadQuotes(new String[] {filename, "2019-12-12"});
  ...
  void mainCalculateAnnualReturn() throws Exception {
    ...
    // then
    List<String> symbols =
        result.stream().map(AnnualizedReturn::getSymbol).collect(Collectors.toList());
  ...
  public void testCalculateAnnualizedReturnGoogl() {
    ...
    AnnualizedReturn returns = PortfolioManagerApplication
        .calculateAnnualizedReturns(LocalDate.parse("2019-12-12"), trade, 1054.00, 1348.00);
